# Troubleshooting Notes — Lab 67 Staging to Exfiltration Investigation
...
|---|---|
| `StagingRxfilLab` typo | Corrected to `StagingExfilLab` |
| `Get-Child-Item` typo | Corrected to `Get-ChildItem` |
| Extra root-level employee file | Final source copy under `Source` used for the investigation |
| Sysmon Event ID 11 no targeted result | Documented telemetry limitation |
| PowerShell Event ID 4104 no targeted result | Documented telemetry limitation |
| Security Event ID 4688 no result | Documented telemetry limitation |
| Sysmon Event ID 1 available | Used as supporting process telemetry |
| Sysmon Event ID 3 available | Used as network context |
| Archive extraction successful | Contents verified |
| Transfer hash matched | Archive integrity confirmed |
| No external destination | Exfiltration not claimed |

## Final Troubleshooting Lesson

The central lesson from Lab 67 is:

> Good DFIR analysis requires separating technical evidence from assumptions about what the activity means.
...
The analyst needs evidence connecting the staged data to an external destination before concluding that exfiltration occurred.
